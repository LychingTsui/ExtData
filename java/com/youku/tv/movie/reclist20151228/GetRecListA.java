	...
	public static class MapClass extends Mapper<LongWritable, Text, Text, Text> {
		...
		protected void map(LongWritable key, Text value, Context context) throws IOException,
			...
			for (int i = 0; i < type.length; i++) {
				...
				if (map.containsKey("\2" + type[i].replaceAll(":", "").replaceAll("锛�", ""))) {
					String[] temp = map.get("\2" + type[i].replaceAll(":", "").replaceAll("锛�", ""))
							.split(",");
					int le = 8;
					if (le > temp.length) {
						le = temp.length;
					...
					for (int j = 0; j < le; j++) {
						...
						if (sim > 0) {
							double d = sim
									/ typemap.get(type[i].replaceAll(":", "").replaceAll("锛�", ""));
