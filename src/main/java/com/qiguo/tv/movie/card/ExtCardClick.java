...
public class ExtCardClick {
	public static class MapClass extends Mapper<LongWritable, Text, Text, Text>{
		...
		InterruptedException{
			...
			if(info[25].endsWith("listClick")){
				if(info[26].equals("cardList") || info[26].equals("cardDetail")){
	...
	ClassNotFoundException{
		...
		Job job = Job.getInstance(conf, "extCardPos");
		job.setJarByClass(ExtCardClick.class);
