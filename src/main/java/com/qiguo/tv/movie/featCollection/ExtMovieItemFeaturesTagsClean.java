...
public class ExtMovieItemFeaturesTagsClean {
	public static class Mapclass extends Mapper<LongWritable, Text, Text, Text>{	
		...
		HashMap<String, String>cleanTagMp = new HashMap<String, String>();
		...
		InterruptedException, IOException{
			...
			HashMap<Integer, Double>featMp = new HashMap<Integer, Double>();//避免重复
				...
				if(infoStr.startsWith("actor")){
					if(infoStr.length() > 6){
						...
						for(Map.Entry<String, Integer> entry : actSet.entrySet()){
							if(actMp.containsKey(entry.getKey()) && !cleanTagMp.containsKey(entry.getKey())){ //去除异常act
					...
					if(infoStr.length() > 8){
						...
						if(directMap.containsKey(direct) && !cleanTagMp.containsKey(direct)){ //去除异常act
					...
					if(infoStr.length() > 5){
						...
						while (subStk.hasMoreTokens()) {     //
							...
							}else if (cleanTagMp.containsKey(tagstr) && !cleanTagMp.get(tagstr).equals("exp") && tagsMap.containsKey(cleanTagMp.get(tagstr))) {
								BigDecimal bgd = new BigDecimal(sc + 0.05*(lenth - start++));
								double sctmp = Double.parseDouble(bgd.setScale(2, BigDecimal.ROUND_HALF_UP).toString());
								if(!featMp.containsKey(tagsMap.get(cleanTagMp.get(tagstr)))){
									featMp.put(tagsMap.get(cleanTagMp.get(tagstr)), sctmp);
								}
							}
						}
		...
		InterruptedException{
			...
			while((line = bs.readLine()) != null){
				...
				if(lintmp.startsWith("t1")){
					...
					if(lintmp.startsWith("tx")){
						cleanTagMp.put(lintmp.substring(2), "exp");
					}else {
						StringTokenizer stk = new StringTokenizer(lintmp, ",");
						String tag = stk.nextToken();
						cleanTagMp.put(tag, stk.nextToken());
					}
				}
			}
			bs.close();
	...
	IntrospectionException, ClassNotFoundException, InterruptedException{
		...
		job.setJarByClass(ExtMovieItemFeaturesTagsClean.class);
