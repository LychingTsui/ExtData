...
/**
 *@info: 用户偏好长度按25截取，key对应值作为特征值  
 */
public class GetPersonalLikesFeatures4 {
				...
				if(likes.startsWith("label")){
					if(likes.length() > 5){
						...
						while (substk.hasMoreTokens() && cnt < cutoff) {
							...
							double keyval = Double.parseDouble(p.substring(loc+1));
	...
	InterruptedException,ClassNotFoundException{
		...
		job.setJarByClass(GetPersonalLikesFeatures4.class);
