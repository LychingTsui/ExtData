...
public class ExtPersonalShowGreaterOne {
		...
		InterruptedException{
			...
			if(vvguidSet.contains(guid)){
				while(stk.hasMoreTokens()){
					...
					int showcnt = Integer.parseInt(dateStr.substring(idx+1).trim());
					if(mv.length() == 32 && showcnt > 1){
						strbuf.append(mv+" ");
					}
				}
	...
	ClassNotFoundException{
		...
		job.setJarByClass(ExtPersonalShowGreaterOne.class);
