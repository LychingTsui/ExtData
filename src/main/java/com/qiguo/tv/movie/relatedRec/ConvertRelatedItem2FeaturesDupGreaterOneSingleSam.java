...
public class ConvertRelatedItem2FeaturesDupGreaterOneSingleSam {
	HashMap<String, String>movieItemsMp = new HashMap<String, String>();
	public ConvertRelatedItem2FeaturesDupGreaterOneSingleSam(String path)
			throws IOException{
	...
	public static int[] getFeatIdx(String path)throws IOException{
		int[] ftidx = {0,0,0};  //area 起始id, movietags 起始id, join起始id； 
		BufferedReader bfr = new BufferedReader(new FileReader(path));
		String line = "";
		int cnt = 0;
		while((line = bfr.readLine()) != null){
			...
			if(cnt == 2){
				...
				ftidx[2] = Integer.parseInt(line.substring(idx + 1).trim());
			}else if (cnt > 4) {
				break;
			}
		}
	...
	public static void main(String[] args)throws IOException{
		...
		int offset = featIdx[2] - featIdx[0]; //  
		int totlen = featIdx[2] + offset -3; // 
		BufferedWriter bfw = new BufferedWriter(new FileWriter(outPth));
		ConvertRelatedItem2FeaturesDupGreaterOneSingleSam crift = new ConvertRelatedItem2FeaturesDupGreaterOneSingleSam(moviePath);
		...
		while((line = bfr.readLine()) != null){
			...
			if(cnt > 1){      //  对训练数据做去燥处理（只保留点击次数和展示次数 > 1的样本）
				...
				if(mvMp.containsKey(id1) && mvMp.containsKey(id2)){
					...
					for(Map.Entry<Integer, Double> entry : ftmp2.entrySet()){
						if(entry.getKey() < featIdx[2] - 2 && entry.getKey() >= featIdx[0]    //2:上映和时间
								&& ftmp1.containsKey(entry.getKey())){ //&& entry.getKey() >= featIdx[0]
							ftmp1.put(entry.getKey() + offset, 1.0);
						}
						BigDecimal bd = new BigDecimal(entry.getValue());    //   对拼接的电影不做 *cnt 操作
