...
public class ConvertRelatedItem2FeaturesFullJoinwithDateDiff {
	HashMap<String, String>movieItemsMp = new HashMap<String, String>();
	public ConvertRelatedItem2FeaturesFullJoinwithDateDiff(String path)
			throws IOException{
	...
	public static void main(String[] args)throws IOException{
		...
		ConvertRelatedItem2FeaturesFullJoinwithDateDiff crift = new ConvertRelatedItem2FeaturesFullJoinwithDateDiff(moviePath);
			...
			if(cnt > 1){    //去除样本中点击和展示次数为1 的样本（去燥）
				...
				if(mvMp.containsKey(id1) && mvMp.containsKey(id2)){
					...
					for(Map.Entry<Integer, Double> entry : ftmp2.entrySet()){
						if(entry.getKey() < featIdx[1]- 2       //上映和打分
