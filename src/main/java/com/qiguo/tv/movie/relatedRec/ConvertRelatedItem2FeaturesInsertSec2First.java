...
public class ConvertRelatedItem2FeaturesInsertSec2First {
	HashMap<String, String>movieItemsMp = new HashMap<String, String>();
	public ConvertRelatedItem2FeaturesInsertSec2First(String path)
			throws IOException{
	...
	public static void main(String[] args)throws IOException{
		...
		ConvertRelatedItem2FeaturesInsertSec2First crift = new ConvertRelatedItem2FeaturesInsertSec2First(moviePath);
			...
			if(cnt > 1){    //去除样本中点击和展示次数为1 的样本（去燥）
				...
				if(mvMp.containsKey(id1) && mvMp.containsKey(id2)){
					...
					for(Map.Entry<Integer, Double> entry : ftmp2.entrySet()){
						if(entry.getKey() < featIdx[1]- 2){       //-2:打分和时间
								if(ftmp1.containsKey(entry.getKey())){ //&& entry.getKey() >= featIdx[0]
									double tmpV = ftmp1.get(entry.getKey());
									BigDecimal bd = new BigDecimal(tmpV + entry.getValue());
									double res = Double.parseDouble(bd.setScale(2, BigDecimal.ROUND_HALF_UP).toString());
									ftmp1.put(entry.getKey(), res);
									ftmp1.put(entry.getKey() + offset, res); //1.0*cnt, 1.0
								}else {
									ftmp1.put(entry.getKey(), entry.getValue());
								}
						}else if (entry.getKey() == featIdx[1] - 1  //把 上映时间 也做交叉特征处理
								&& ftmp1.containsKey(entry.getKey())) {
							ftmp1.put(totlen, getDateDiff(entry.getValue(), ftmp1.get(entry.getKey())));
						}
