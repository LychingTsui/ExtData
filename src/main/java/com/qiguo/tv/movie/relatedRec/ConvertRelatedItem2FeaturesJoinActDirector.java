...
public class ConvertRelatedItem2FeaturesJoinActDirector {
	public static class MapClass extends Mapper<LongWritable, Text, Text, NullWritable>{
		...
		InterruptedException{
			...
			if(movieItemFeatsMp.containsKey(mvstr1) &&
				...
				int offset = featIdx[1]-1; // offset为tags 键的长度,含上映和打分 
				int totlen = featIdx[1] -1+ featIdx[1]-3; 
	...
	InterruptedException,ClassNotFoundException{
		...
		MultipleInputs.addInputPath(job, new Path(argsStrs[0]), TextInputFormat.class,
				MapClass.class);
		job.setJarByClass(ConvertRelatedItem2FeaturesJoinActDirector.class);
