...
/**
 *只对第二部电影的act，direct，tags做重复倍数处理，
 *对年份和评分不做重复倍数处理
 **/

public class ConvertRelatedItem2FeaturesSecMvNoYrRate {
	HashMap<String, String>movieItemsMp = new HashMap<String, String>();
	public ConvertRelatedItem2FeaturesSecMvNoYrRate(String path)
			throws IOException{
	...
	public static void main(String[] args)throws IOException{
		...
		int totlen = featIdx[1] + offset - 3; //第一部电影＋交叉tgas键特征的总长度
		BufferedWriter bfw = new BufferedWriter(new FileWriter(outPth));
		
		ConvertRelatedItem2FeaturesSecMvNoYrRate crift = new ConvertRelatedItem2FeaturesSecMvNoYrRate(moviePath);
		...
		while((line = bfr.readLine()) != null){
			...
			if(mvMp.containsKey(id1) && mvMp.containsKey(id2)){
				...
				for(Map.Entry<Integer, Double> entry : ftmp2.entrySet()){
					...
					if(entry.getKey() > featIdx [1] - 3){  //只对第二部电影的act direct tags做倍数处理,不对年份和评分特征做倍数处理
