...
public class TopKPredByRelatedRecModelJoinAreaTags {
	public static class MapClass extends Mapper<LongWritable, Text, Text, NullWritable>{
		...
		private int[] featIdx = {0,0};  // featidx[0]: area起始id，featidx[1]:join 起始id
		...
		InterruptedException{
			...
			for(Map.Entry<String, String>entry : movieItemFeatsMp.entrySet()){
				if(!entry.getKey().equals(mvid)){
					HashMap<Integer, Double> feats = getFeatures(mvFeatStr, entry.getValue());
		...
		public double predScore(HashMap<Integer, Double>feats, HashMap<Integer, Double>weiMp){
			
			double res = 0.0;
			for(Map.Entry<Integer, Double> entry : feats.entrySet()){
				res += entry.getValue() * weiMp.get(entry.getKey());
			}
			BigDecimal bd = new BigDecimal(res);
	        res = Double.parseDouble(bd.setScale(2, BigDecimal.ROUND_HALF_UP).toString());
			return res;
		}
		public HashMap<Integer, Double> getFeatures(String str1, String str2){
	...
	InterruptedException, ClassNotFoundException{
		...
		conf.set("featIdx1", otherArgs[4]);  // area起始id
		conf.set("featIdx2", otherArgs[5]);  // join 起始id
		...
		job.setJarByClass(TopKPredByRelatedRecModelJoinAreaTags.class);
