...
import com.qiguo.tv.movie.relatedRecXgModel.ConvertRelatedItem2FeaturesFullJoin.MapClass;

public class ConvertRelatedItem2Features {
		...
		InterruptedException{
			...
			if(movieMp.containsKey(mv1) && movieMp.containsKey(mv2)){
				...
				while (subStk.hasMoreTokens()) {
					...
					if(p2.getIdx() < featIdxArr[0]- 2){       //打分，上映
							if(mv1Mp.containsKey(p2.getIdx())){ //&& entry.getKey() >= featIdx[0]
								mv1Mp.put(p2.getIdx() + offset, 1.0);
								double tmpScore = mv1Mp.get(p2.getIdx());
								mv1Mp.put(p2.getIdx(), tmpScore + p2.getScore());
							}else {
								mv1Mp.put(p2.getIdx(), p2.getScore());
							}
	...
	InterruptedException,ClassNotFoundException{
		...
		job.setJarByClass(ConvertRelatedItem2FeaturesFullJoin.class);
