...
public class ConvertRelatedItem2FeaturesFullJoinClkN {
	public static class MapClass extends Mapper<LongWritable, Text, Text, NullWritable>{
		...
		InterruptedException{
			...
			int clkN = Integer.parseInt(stk.nextToken()); 
			...
			if(movieMp.containsKey(mv1) && movieMp.containsKey(mv2)){
				...
				while (subStk.hasMoreTokens()) {
					...
					if(p2.getIdx() < featIdxArr[0]- 2       //上映和时间
						...
						mv1Mp.put(p2.getIdx() + offset, 1.0 * clkN); // *clkN
					...
					double tmpRes = p2.getScore()*clkN;
					BigDecimal bgd = new BigDecimal(tmpRes);
					tmpRes = Double.parseDouble(bgd.setScale(2,BigDecimal.ROUND_HALF_UP).toString());
					mv1Mp.put(p2.getIdx() + totlen, tmpRes);
				}
	...
	InterruptedException,ClassNotFoundException{
		...
		job.setJarByClass(ConvertRelatedItem2FeaturesFullJoinClkN.class);
