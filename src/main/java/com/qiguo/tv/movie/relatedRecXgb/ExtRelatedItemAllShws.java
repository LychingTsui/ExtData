...
public class ExtRelatedItemAllShws {
								...
								if(js.has("algGroup") && !js.get("algGroup").toString().isEmpty()){
									if(js.has("list")){
										...
										for(int i = 0; i < jsar.length(); i++){
											...
											context.write(new Text(mvid + ":" + disMvId), new IntWritable(1));
										}
										context.write(new Text(mvid), new IntWritable(1));
									}
								}
							}
	...
	ClassNotFoundException{
		...
		job.setJarByClass(ExtRelatedItemAllShws.class);
