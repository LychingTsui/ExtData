...
public class ExtRelatedItemClks {
						...
						try {
							...
							if(flag){
								if(js.has("algGroup") && !js.get("algGroup").toString().isEmpty()){
									if(js.has("videoInfo") && js.has("refVideoInfo")){
										JSONObject vjs = new JSONObject(js.getString("videoInfo"));
										String vMvId = vjs.getString("id");
										JSONObject refjs = new JSONObject(js.getString("refVideoInfo"));
										String refMvId = refjs.getString("id");
										String pair = refMvId + ":" + vMvId;
	...
	ClassNotFoundException{
	    ...
	    if(fstas.isDirectory()){
				for(FileStatus subfstus : dirfs.listStatus(new Path(argstrs[0]))){
					...
					if(flag1 && flag2){
						...
						//FileStatus[] secfst = f.listStatus(new Path(subdir));
		...
		job.setJarByClass(ExtRelatedItemClks.class);
