...
public class ComicRelatedDisplayUvSts {
					...
					if(isJson(info[27])){
						...
						try {
							...
							if(js.has("refVideoInfo")){
								...
								if(isJson(refVideo)){
									JSONObject subJs = new JSONObject(refVideo);
									if(subJs.has("id")){
										String mvid = subJs.getString("id").toString();
										String text = mvid +" "+info[15];
	...
	InterruptedException,ClassNotFoundException{
		...
		Job job = Job.getInstance(conf, "comicDisplayRelated:uv");
		job.setJarByClass(ComicRelatedDisplayUvSts.class);
