...
public class MovieCardDisplayUvSts {
					...
					if(isJson(info[27])){
						...
						try {
							...
							if(js.has("list")){
								...
								for(int i = 0; i<len; i++){
									if(!names[i].equals("personalRec")){
										String subjs = new JSONObject(js.getString("list")).get(names[i]).toString();
	...
	InterruptedException,ClassNotFoundException{
		...
		job.setJarByClass(MovieCardDisplayUvSts.class);
