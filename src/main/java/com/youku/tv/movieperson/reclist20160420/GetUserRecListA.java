...
@Override
           ...
           //是否有估算，保证每个用户都能达到可用推荐数
			while (it.hasNext()) {
				...
				if (fvalue.startsWith("auser")) {
					...
					for (Entry<String, Double> maps:userscore.entrySet()) {
						double d=Double.valueOf(data[0])*maps.getValue();
						if (d < cutoff) {
							c.increment(1);
								continue;
							}
							else if (list.contains(data[1]) && d<cutdown) {
								c1.increment(1);
								continue;
							}
		...
		protected void setup(Context context) throws IOException, InterruptedException {
			...
			cutdown=Double.valueOf(context.getConfiguration().get("cutdown"));
			super.setup(context);
