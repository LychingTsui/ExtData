...
public class IDToName2 {
	public static class MapClass extends Mapper<LongWritable, Text, Text, Text> {
		...
		InterruptedException {
			...
			for (int i = 1; i < info.length; i++) {
				String da[]=info[i].split(",");
				if (map.containsKey(da[0])) {
					buffer.append(map.get(da[0])+":"+da[da.length-1]+",");
				}
				else{
					buffer.append(da[0]+":"+da[da.length-1]+",");
				}
			}
	...
	ClassNotFoundException {
		...
		job.setJarByClass(IDToName2.class);
